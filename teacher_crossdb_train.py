...
def parse_args():
    ...
    parser.add_argument('--snapshot', dest='snapshot', help='Path of model snapshot.',
                        default='model_cmp_base', type=str)
...
if __name__ == '__main__':
    ...
    if database_train == 'CVIQ':
        ...
        file_train = './data/OIQA/OIQA.csv'
    elif database_train == 'IQA-ODI':
        file_train = './data/IQA-ODI/ODI_1080.csv'
        
    if database_test == 'CVIQ':
        file_test = './data/CVIQ/CVIQ.csv'
    elif database_test == 'OIQA':
        file_test = './data/OIQA/OIQA.csv'
    elif database_test == 'IQA-ODI':
        file_test = './data/IQA-ODI/ODI_1080.csv'
    ...
    train_dataset = dataset_viewport.Dataset(database_train, file_train)
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True, num_workers=6)
    test_dataset = dataset_viewport.Dataset(database_test, file_test)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if abs(val_SRCC) + abs(val_PLCC) > best_val_criterion:
                print("Update best model using best_val_criterion in epoch {}".format(epoch + 1))
                best_val_criterion = abs(val_SRCC) + abs(val_PLCC)
                best_val = [val_SRCC, val_KRCC, val_PLCC, val_RMSE]
                
                if not os.path.exists('./cross_database'):
                    os.mkdir('./cross_database')
                torch.save(model.state_dict(), os.path.join('./cross_database/' + snapshot + '_' + database_train + '_' + database_test 
                                                            + '_epoch' + str(epoch+1) + '_' 
                                                            + str('{:.4f}_{:.4f}_{:.4f}'.format(val_SRCC, val_PLCC, val_RMSE)) + '.pkl'))
        ...
        print('Viewport model Training on {} and Testing on {} completed.'.format(database_train, database_test))
